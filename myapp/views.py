...
def checkout(request):
	...
	return render(request,'checkout.html',{'cart':cart,'user':user,'net_price':net_price})
...
def seller_product_detail(request,ab):
	product=Product.objects.get(pk=ab)
	return render(request,'seller_product_detail.html',{'product':product})

def seller_edit_product(request,ab):
	product=Product.objects.get(pk=ab)
	if request.method=="POST":
...
def seller_delete_product(request,ab):
	product=product.objects.get(ab=pk)
	product.delete()
	return redirect('seller_view_product')

def product_details(request,ab):
	wishlist_flag=False
	cart_flag=False
	product=Product.objects.get(pk=ab)
	user=User.objects.get(email=request.session['email'])
...
def add_to_wishlist(request,ab):
	product=Product.objects.get(pk=ab)
	user=User.objects.get(email=request.session['email'])
...
def remove_from_wishlist(request,ab):
	product=Product.objects.get(pk=ab)
	user=User.objects.get(email=request.session['email'])
...
def remove_from_cart(request,ab):
	user=User.objects.get(email=request.session['email'])
	product=Product.objects.get(pk=ab)
...
def change_qty(request,ab):
	cart=Cart.objects.get(pk=ab)
